...
Shader "Custom/GrassQuad_Flower_start"
{
	Properties
	{
		...
		_RandomHeightScale("Random Height Scale", Range(0, 3)) = 1
		// flower
		_FlowerSize("Flower Size", Range(0, 5)) = 1
		// fall range
	...
	void geom(point vertIn p[1], inout TriangleStream<geomOut> triStream)
	{
		...
		// 随机位置
		//p[0].pos = p[0].pos + float4(rand(v.texcoord.x + v.texcoord.y), 0,  rand(v.texcoord.x * 2 + v.texcoord.y * 3), 0);
		
		//o._ShadowCoord = ComputeScreenPos(mul(unity_ObjectToWorld, p[0].pos));

		// 随机风
		//float2 wind = float2(_CosTime.z + frac(p[0].pos.x) + 1, _SinTime.z + frac(p[0].pos.x) + 1);
		...
		// //float3 dir = normalize((height * up + randomDir * _RandomDirScale));
		// //float3 dir = normalize(height * up + windVec * _WindScale + randomDir * _RandomDirScale);

		// 随机+风影响后的顶点
		float3 Pt = p[0].vertex + _BaseHeight * up;
